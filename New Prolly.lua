...
--local OOP = require(script.OOP) -- REMOVED: Inlining module
local WaypointClearConfig = require(workspace.Actor.WayPointClear.Config)
local TweenService = game:GetService("TweenService") -- Added for OOP module

--settings
local VisualRayEnabled = false
local VisualLPSEnabled = true

--- OOP Module Code (Inlined) ---
local OOP = {}
OOP.__index = OOP
...
--roam waypoints
OOP.RoamWayPoints = workspace.Waypoints

--filters any glass and the NPC itself
OOP.FilterTable = {script.Parent}
local RayParams = RaycastParams.new()
RayParams.FilterType = Enum.RaycastFilterType.Exclude
RayParams.FilterDescendantsInstances = OOP.FilterTable

--constructor
function OOP.New(model, VisualRayCast, VisualLPS)
...
function OOP.GetCharacter(part)
	if part and part.Parent and part.Parent.Parent then
		...
		elseif humparentparent then
			return humparentparent.Parent
		end
	end
end

function OOP.IsPlayerInTable(Table)
	for i,v in pairs(Table) do
		-- v is {RaycastResult}
		if OOP.GetCharacter(v[1].Instance) then 
			return true
		end
	end
	return false
...
function OOP.GetTableLength(Table)
	local totalnumber = 0
	for i,v in Table do
		totalnumber+=1
	end
...
function OOP:Create_Cast(deg)
	local RayResult = workspace:Raycast(self.Head.Position, self.Head.CFrame:ToWorldSpace(CFrame.Angles(math.rad(OOP.Tilt), math.rad(deg), 0)).LookVector * OOP.FovDistance, RayParams)
	local HitPosition = RayResult and RayResult.Position or self.Head.Position + self.Head.CFrame:ToWorldSpace(CFrame.Angles(math.rad(OOP.Tilt), math.rad(deg), 0)).LookVector * OOP.FovDistance
	
	return RayResult, HitPosition
end

--turns the npc head 180 degrees and back (not currently used, planning on implementing it later)
function OOP:TurnHead()
...
--DEPRECATED METHOD
function OOP:CreateRayCast(deg)
...
function OOP:CreateVectors(Hitpos)
...
function OOP:CreateVisualRayCast(HitPosition, MiddlePoint, Distance, ID)
	local VisualRay = Instance.new("Part")
	table.insert(OOP.FilterTable, VisualRay)
	RayParams.FilterDescendantsInstances = OOP.FilterTable
	VisualRay.Anchored = true
	VisualRay.CanCollide = false
	VisualRay.Size = Vector3.new(0.1, 0.1, Distance or OOP.FovDistance)
...
function OOP:CreateRayCasts(amount)
	...
	--angle per each cast
	local AnglePerEachCast = OOP.TOTAL_ANGLE / amount
	...
	local CurrentAngle = OOP.TOTAL_ANGLE / 2
...
function OOP:CreateVisualLPS()
	local LastPartSeen = Instance.new("Part")
	table.insert(OOP.FilterTable, LastPartSeen)
...
--raycast options
local FovDistance = OOP.FovDistance -- uses the inlined module setting
						...
						for i,v:PathWaypoint in Waypoints do
							...
							if i ~= 1 then
								...
								if TargetCharacter then
									...
									-- Use waypos which is LastPositionPlayerWasSeen
									Hum:MoveTo(waypos) 
								end
								--jump if needed
...
while task.wait(0.05) do
	...
	for i, Raycast in SuccessRays do
		if Raycast[1] then
